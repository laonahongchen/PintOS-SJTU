...
static bool priority_more (const struct list_elem *lhs, const struct list_elem *rhs, void *aux UNUSED);
...
{
  int ready_threads = list_size (&ready_list) + (thread_current ()->status != idle_thread);
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  old_level = intr_disable ();
  list_insert_ordered (&all_list, &t->allelem, priority_more, NULL);
  intr_set_level (old_level);
}
...
static bool
priority_more (const struct list_elem *lhs, const struct list_elem *rhs, void *aux UNUSED)
{
  struct thread *a, *b;
